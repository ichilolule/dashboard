...
- 朝昼／夜は開始時刻を推測せず終日イベントとする。全イベントは非公開・空き時間扱い・通知なし。
...
- 転記対象は案件名・工程・期日・返信待ち。顧客連絡先、住所、銀行情報、料金、請求書や本文メモ、過去案件全体は送らない。
...
7. Google側の専用カレンダー作成、転記、再転記時の非重複、変更・削除・オフライン再開を実機で確認する。
